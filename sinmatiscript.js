window.addEventListener('DOMContentLoaded', () => {
	...
	const frame1 = document.getElementById("kameratoukawaku");
	const frame2 = document.getElementById("shinmarukun");
	const frameRadios = document.querySelectorAll('input[name="frame_choice"]');
	...
	function updateLiveFrame() {
		...
		// 選択に応じてフレームの表示・非表示を切り替え
		if (selectedValue === "kameratoukawaku.png") {
	...
	// ラジオボタンが変更されたら、ライブプレビューのフレームを更新
	frameRadios.forEach(radio => {
		radio.addEventListener('change', () => {
			if (cameraStream) { // カメラが起動している時だけ更新
				updateLiveFrame();
			}
		});
	});
...
//		kameratoukawaku.style.display = "none"; // カメラ枠非表示
	...
	if (shutterButton) {
        ...
        // 1. 選択されているラジオボタンから、使用するフレーム画像のファイル名を取得
			...
			// 2. Canvasに動画の現在のフレームを描画
			context.drawImage(video, 0, 0, photoCanvas.width, photoCanvas.height);

			// 3. ユーザーが選択したフレーム画像を読み込み、Canvasに描画
			const frameImage = new Image();
			frameImage.src = selectedFrameSrc; // 選択された画像ソースを使用

			// 4. 画像の読み込みが成功した時の処理
			frameImage.onload = () => {
				// 枠画像のサイズを調整 (動画の幅の50%に)
			...
			// 6. 画像の読み込みが失敗した時の処理
			frameImage.onerror = () => {
				alert("選択されたフレーム画像の読み込みに失敗しました。枠画像なしでダウンロードします。");
				// 枠なしでダウンロードするフォールバック処理
				photoCanvas.toBlob((blob) => {
					if (blob) {
						const url = URL.createObjectURL(blob);
						const a = document.createElement('a');
						const now = new Date();
						const filename = `aomori_photo_noframe_${now.getFullYear()}${(now.getMonth() + 1).toString().padStart(2, '0')}${now.getDate().toString().padStart(2, '0')}_${now.getHours().toString().padStart(2, '0')}${now.getMinutes().toString().padStart(2, '0')}${now.getSeconds().toString().padStart(2, '0')}.png`;
	...
	const shops = [
		...
		{id: 3,sid: 6,jyuusyo: "青森市新町2-8-4",sname: "喫茶クレオパトラ",oname: "バナナとりんごのケーキ"},
		{id: 3,sid: 6,jyuusyo: "青森市新町2-8-4",sname: "喫茶クレオパトラ",oname: "レスキュースープ"},
		{id: 3,sid: 6,jyuusyo: "青森市新町2-8-4",sname: "喫茶クレオパトラ",oname: "英国風スコーンセット"},
		...
		{id: 6,sid: 10,jyuusyo: "青森市新町1-9-22",sname: "鮨処あすか",oname: "サーモン塩麹焼き"},
		{id: 6,sid: 10,jyuusyo: "青森市新町1-9-22",sname: "鮨処あすか",oname: "自家製茶碗蒸し"},
		{id: 6,sid: 10,jyuusyo: "青森市新町1-9-22",sname: "鮨処あすか",oname: "ばらちらし"},
		{id: 6,sid: 11,jyuusyo: "青森市新町1-11-16 ダイワロイネットホテル青森 1F",sname: "うぐいす",oname: "特製おまかせ握り9貫※事前予約必須"},
		{id: 6,sid: 11,jyuusyo: "青森市新町1-11-16 ダイワロイネットホテル青森 1F",sname: "うぐいす",oname: "THE OBANZAI"},
		{id: 6,sid: 11,jyuusyo: "青森市新町1-11-16 ダイワロイネットホテル青森 1F",sname: "うぐいす",oname: "UGUISUの極上牛まぶし膳"},
		{id: 6,sid: 12,jyuusyo: "青森市新町1-13-5",sname: "チャンドラ",oname: "エンガデン"},
		{id: 7,sid: 8,jyuusyo: "青森市古川1丁目16-1 一郎屋ビル 1F",sname: "True",oname: "Trueカレー"},
		{id: 7,sid: 8,jyuusyo: "青森市古川1丁目16-1 一郎屋ビル 1F",sname: "True",oname: "2色カレー"},
		{id: 7,sid: 9,jyuusyo: "青森市古川1丁目17-1",sname: "COFFEEMAN good",oname: "コーヒーを通じてつながるコミュニティスタンド"},
